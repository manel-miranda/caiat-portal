-- 1. Reset: every DELETE gets an explicit WHERE clause
...
-- 2. Historical demo data generator (admin only, idempotent per run id)
...
BEGIN
  ...
  -- demo suppliers, created once, never overwriting user-created rows
  ...
  -- plausible unit costs per ingredient (deterministic)
  ...
  -- daily meal plan across the demo recipes
  ...
  SELECT u.item_id,
         ...
         'simulation_history',
         uuid_generate_v5_compat(p_run_id, 'day-' || u.day),
         auth.uid(),
  ...
  -- six backdated purchases covering roughly what was consumed
  FOR v_k IN 0..5 LOOP
    v_ts := now() - make_interval(days => (v_days - v_k * 5));
    v_purchase_id := uuid_generate_v5_compat(p_run_id, 'purchase-' || v_k);
...
REVOKE ALL ON FUNCTION public.inventory_simulate_history(uuid, integer) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.inventory_simulate_history(uuid, integer) TO authenticated, service_role;