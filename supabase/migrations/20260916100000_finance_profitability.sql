...
BEGIN
  ...
  SELECT coalesce(jsonb_agg(jsonb_build_object(
   ...
   WHERE i.active = true AND (p_include_preview OR NOT i.preview_only);
  ...
  SELECT coalesce(jsonb_agg(jsonb_build_object(
    ...
    FROM (
      SELECT st.id,
             ...
             CASE WHEN count(rc.id) = count(c.last_unit_cost)
       ...
       WHERE st.active = true AND (p_include_preview OR NOT st.preview_only) AND st.billable = true
